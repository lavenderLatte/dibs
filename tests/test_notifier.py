...
def test_is_quiet_hours_uses_custom_bounds():
    # Custom bounds: 10pm–7am (22–7)
    assert is_quiet_hours("America/Los_Angeles", now=datetime(2026, 1, 1, 22, 0), quiet_start=22, quiet_end=7) is True
    assert is_quiet_hours("America/Los_Angeles", now=datetime(2026, 1, 1, 21, 59), quiet_start=22, quiet_end=7) is False
...
def test_send_push_raises_on_http_error(mocker):
    mock_post = mocker.patch("notifier.requests.post")
...
def test_send_alert_reads_quiet_hours_from_config(mocker):
    ...
    mock_is_quiet = mocker.patch("notifier.is_quiet_hours", return_value=False)
    mocker.patch("notifier.send_email")
    mocker.patch("notifier.send_push")

    from notifier import send_alert
    send_alert(config_with_qh, CREDS, [VACANCY_1])

    mock_is_quiet.assert_called_once_with("America/Los_Angeles", quiet_start=22, quiet_end=7)
